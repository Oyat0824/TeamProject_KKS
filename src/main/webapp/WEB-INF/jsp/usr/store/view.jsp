...
function UserLike__getUserLike() {
	
	$.get('../userLike/UserLike', {
		id : params.id,
		ajaxMode : 'Y'
	}, function(data){
		if(data.data1.UserLike> 0){ // 리액션 버튼만 교체하는 역할
			let pickBtn = $('#pickBtn'); 
			pickBtn.removeClass('btn-outline');
			pickBtn.prop('href', '../userLike/doUserLike?id=${store.id}&liked=1')
		}else if(data.data1.UserLike < 0){
			let delBtn = $('#delBtn');
			delBtn.removeClass('btn-outline');
			delBtn.prop('href', '../userLike/delUserLike?id=${store.id}&liked=-1')
		}
	}, 'json');
}
	...
	<div class="container mx-auto px-3">
		...
		<div class="btns flex justify-between mt-5">
			...
			<c:if test="${rq.getLoginedMemberId() != 0 }"> <!-- 로그인 여부. 로그인 후에 찜 클릭 가능-->
  				<a class="btn" href="../userLike/doUserLike?id=${store.id}&relTypeCode=store&liked=1"><i class="fa-solid fa-heart mr-1"></i></a>
  				<span class="badge">${store.userLike}</span>
  				<c:if test="${store.userLike != 0 }">
   					<a class="btn" href="../userLike/delUserLike?id=${store.id}&relTypeCode=store&liked=-1"><i class="fa-solid fa-heart mr-1"></i></a>
